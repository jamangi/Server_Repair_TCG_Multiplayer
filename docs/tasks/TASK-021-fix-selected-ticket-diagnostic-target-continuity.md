...
- Retaining the diagnostic's visual selection is allowed, but its action area must change to a calm selected-Ticket status such as `Completed for this machine revision` or a player-safe generic `Not currently runnable on this Ticket`; it must not offer another Ticket automatically.
...
- a completed/current-revision diagnostic remains inspectable and cannot be resubmitted on that Ticket;
