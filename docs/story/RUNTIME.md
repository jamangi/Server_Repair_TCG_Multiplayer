...
The current Story content version is `quiet-cascade-expansion-v3` under the unchanged `story.campaign.quiet_cascade.v1` pack identity. Its migration accepts only the reviewed v1→v2→v3 and v2→v3 predecessor chains. Before carrying anything forward, the client loads the complete predecessor bundles and restores the original checkpoint against its original digest. It preserves scene/Match boundary, variables, choices, Story Service Points, branch history, accepted results, and any pending normalized result; adds only the two new registry defaults; and computes a new v3 digest. A completed campaign-one marker is cleared only after its exact old ending remains derivable from the preserved points band and ending checkpoint ID, because that checkpoint now resumes at `story.qc02.entry` rather than terminating. No Match result, reward, choice, or replay record is synthesized.
...
TASK-027 through TASK-030 own the campaign-one foundation. TASK-038 owns isolated completed-episode review. TASK-046 owns the additive `quiet-cascade-expansion-v3` release, predecessor migration, current catalog/Worker bridge, twelve-episode portability, and post-release coverage boundary.
