...
TASK-037 completed the final Shift 6 repair, and TASK-038 made each durably completed Shift replayable as an isolated cutscene plus ordinary practice Match. TASK-039 through TASK-046 continue with exact campaign/domain coverage; an evidence-gated expansion protocol; minimum-Q case research; coherent domain integration; a six-episode Story+Match blueprint; context-complete scripts; gap-only original art; and one versioned release. In this sequence, an **episode** means one player-facing story sequence plus one real Match. That terminology avoids conflating the current campaign's four script chapters with its six Shifts/Matches. See the [task index](../tasks/INDEX.md) for the active task, exact order, and dependencies.
