...
Status: **implemented canonical campaign one plus the live sourced `quiet-cascade-expansion-v3` release; no story document is gameplay-rule authority**
...
Campaign-one characterization remains preserved in `content/story-v1/campaigns/quiet-cascade-characterization-v2/`, while production now loads the additive `content/story-v1/campaigns/quiet-cascade-expansion-v3/`. The original `quiet-cascade/` and characterization-v2 directories remain immutable migration and comparison history. See the [final dialogue comparison](revisions/quiet-cascade-characterization-v2/FINAL_DIALOGUE_COMPARISON.md), [48 side-by-side route transcripts](revisions/quiet-cascade-characterization-v2/FINAL_ROUTE_TRANSCRIPTS.md), [character reference ledger](CHARACTER_REFERENCE_LEDGER.md), and [voice cards](CHARACTER_VOICE_CARDS.md).
...
TASK-037 completed the final Shift 6 repair, TASK-038 made each durably completed Shift replayable as an isolated cutscene plus ordinary practice Match, and TASK-039 published the exact [campaign-one domain coverage audit](coverage/CAMPAIGN_ONE_DOMAIN_COVERAGE.md). TASK-040 established the twelve-gate [Story and domain expansion protocol](EXPANSION_PROTOCOL.md), and TASK-041 calculated `Q = 6` in the [six-slot research evidence matrix](../case_studies/v0.2-story-expansion/SIX-SLOT-EVIDENCE-MATRIX.md). TASK-042 then integrated those six cases into the versioned [expansion domain network](../coverage/TASK-042-EXPANSION-DOMAIN-NETWORK.md): 18 total fingerprints, 83 playable Cards, an unchanged legacy starter deck, and six deterministic Builder/engine proofs. TASK-043 completed the non-live [QC02 beat sheets](revisions/quiet-cascade-expansion-v3/BEAT_SHEETS.md), [graph proof](revisions/quiet-cascade-expansion-v3/GRAPH_REPORT.json), [256-route matrix](revisions/quiet-cascade-expansion-v3/ROUTE_ENDING_MATRIX.md), deck/coverage forecast, and reuse-only art plan. TASK-044 delivered the context-complete [candidate pack](../../content/story-v1/candidates/quiet-cascade-expansion-v3/manifest.json), source trace, canonical route transcripts, and zero-gap art brief; TASK-045 proved all 70 reachable visual references reuse 4 approved backgrounds and 8 approved poses with no new production pixels. TASK-046 then released the additive pack, safe progress migration, twelve replay boundaries, and deliberate current-content ending. Its [release record](releases/quiet-cascade-expansion-v3/RELEASE_CONTENT_PACK.md), [post-release coverage audit](coverage/RELEASED_STORY_DOMAIN_COVERAGE_V3.md), and [browser QA](TASK-046-BROWSER-QA.md) are the current proof boundary. Here an **episode** means one player-facing Story sequence plus one real Match; the release contains twelve episodes and 18 Tickets. See the [task index](../tasks/INDEX.md) for the completed dependency chain.

[`STORY-009 A`](../design/decisions/APPROVALS.md#story-009--completed-episode-replay-semantics--a-approved-2026-08-28) governs the completed replay path: prior cutscenes and their real configured Matches may be practiced without rewriting canonical progress, accepting another canonical result, or changing Profile statistics.
