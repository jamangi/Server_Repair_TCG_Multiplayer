...
| Field | Value |
| --- | --- |
| Dossier | `model.loose-storage-cable.v1` |
| Released Ticket | `ticket.generated.5352abd871c2e9076be92a0b` |
| Fingerprint | `fingerprint.storage.loose_cable` |
| Ticket snapshot SHA-256 | `b06dc5a7d12c29af78edd71f7a82a611d971844e6e71863e142bc293f4e6af27` |
...
- Symptoms: `symptom.storage.io_errors`
- Candidates: `fault.storage.backplane.path_failed`, `fault.storage.cable.failed`, `fault.storage.cable.loose`, `fault.storage.nvme.device_failed`, `fault.storage.sas.drive_failed`
- Focus: Hybrid backplane, data-path service boundaries, device inventory, and post-reseat discovery
- Public-safe statement: The visible model exposes devices, cable/path abstractions, backplane, and controller boundaries so every public storage Candidate remains physically possible.
...
- Hidden authored Faults: `fault.storage.cable.loose`
- Isolation route kinds: `CORROBORATED_SUPPORT`
- Repair procedures: `repair.storage.reseat_cable`
- Verification procedures: `verify.storage.device_detected`
- Consistency result: The private view can bind the authored loose-cable instance to one documented reconnectable branch, corroborate a physical observation with an inventory observation, reseat the connection under service procedure, and then verify device discovery. The public view deliberately shows both cable branches and does not disclose which connector the Ticket binds privately.
...
**Generated public description:** Standby power reaches the management domain. BMC and lifecycle services initialize independently of the host OS. A host-power request enables the switched board path. Host firmware initializes processor, memory, adapters, and device inventory. UEFI applies boot policy to an available boot option. Firmware hands control to the selected OS loader. The OS exercises storage and network paths while management continues telemetry and event collection. The visible model exposes devices, cable/path abstractions, backplane, and controller boundaries so every public storage Candidate remains physically possible.
...
[Open the standalone SVG](../diagrams/02-loose-storage-cable.svg). The SVG has a title and long description, labeled relation types, visible keyboard focus for semantic component groups, patterns/markers that survive monochrome or forced-colors rendering, and a deterministic view box.

**Text equivalent:** Ticket ticket.generated.5352abd871c2e9076be92a0b has public Symptoms symptom.storage.io_errors. Its public Candidates are fault.storage.backplane.path_failed, fault.storage.cable.failed, fault.storage.cable.loose, fault.storage.nvme.device_failed, fault.storage.sas.drive_failed. The schematic focus contains System board (host firmware post); BMC + lifecycle (management); Hybrid backplane (storage); SAS cable + PERC (storage); PCIe / NVMe path (storage); 20 SAS/SATA + 4 NVMe (storage); OS loader + runtime (os handoff). Connections, in reading order: control relation from System board to BMC + lifecycle: host management; power relation from System board to Hybrid backplane: power + signal; data relation from System board to SAS cable + PERC: host storage; data relation from SAS cable + PERC to Hybrid backplane: SAS branch; data relation from System board to PCIe / NVMe path: PCIe branch; data relation from PCIe / NVMe path to Hybrid backplane: NVMe branch; containment relation from Hybrid backplane to 20 SAS/SATA + 4 NVMe: mounted bays; control relation from BMC + lifecycle to Hybrid backplane: inventory / events; data relation from OS loader + runtime to 20 SAS/SATA + 4 NVMe: driver / I/O. Line labels and dash patterns distinguish relation types; no node or edge states which Candidate is true.
...
| Command | `command.linux.smartctl` | OS loader + runtime → SAS cable + PERC → 20 SAS/SATA + 4 NVMe | queries a reachable controller/device health surface |
...
| Repair | `repair.storage.reseat_cable` | SAS cable + PERC → PCIe / NVMe path → Hybrid backplane | intervenes on a documented reconnectable storage interconnect boundary |
...
| --- | --- | --- | --- | --- |
| 1 | RUN_DIAGNOSTIC | test.general.visual_inspection | SAS cable + PERC → PCIe / NVMe path → Hybrid backplane | evidence.fingerprint.storage.loose.cable.machine.storage.loose.cable.active.test.general.visual.inspection |
| 2 | RUN_DIAGNOSTIC | test.storage.device_inventory | BMC + lifecycle → OS loader + runtime → 20 SAS/SATA + 4 NVMe | evidence.fingerprint.storage.loose.cable.machine.storage.loose.cable.active.test.storage.device.inventory |
| 3 | COMMIT_ISOLATION | fault_instance.storage.cable | SAS cable + PERC → PCIe / NVMe path → Hybrid backplane | fault_instance.storage.cable |
| 4 | PERFORM_REPAIR | repair.storage.reseat_cable | SAS cable + PERC → PCIe / NVMe path → Hybrid backplane | repair_outcome.fingerprint.storage.loose.cable |
| 5 | PERFORM_VERIFY | verify.storage.device_detected | BMC + lifecycle → OS loader + runtime → 20 SAS/SATA + 4 NVMe | verify_outcome.fingerprint.storage.loose.cable.01.pass |
...
| --- | --- | --- |
| `fault.storage.backplane.path_failed` | Hybrid backplane | The documented replaceable hybrid backplane is a distinct service boundary. |
| `fault.storage.cable.failed` | SAS cable + PERC → PCIe / NVMe path | The configuration has separate documented SAS and PCIe/NVMe interconnect branches. |
| `fault.storage.cable.loose` | SAS cable + PERC → PCIe / NVMe path → Hybrid backplane | Manufacturer routing and service procedures establish reconnectable cable/connector boundaries. |
| `fault.storage.nvme.device_failed` | 20 SAS/SATA + 4 NVMe → PCIe / NVMe path | Replaceable NVMe devices are present in the fixed hybrid option. |
| `fault.storage.sas.drive_failed` | 20 SAS/SATA + 4 NVMe → SAS cable + PERC | SAS/SATA bays and a controller-attached SAS branch are present. |

Differential variants tested with this exact public input: `fault.storage.backplane.path_failed`, `fault.storage.cable.failed`, `fault.storage.cable.loose`, `fault.storage.nvme.device_failed`, `fault.storage.sas.drive_failed`. The focused validator substitutes each Candidate as synthetic hidden truth and proves the public narrative, text equivalent, and SVG remain byte-identical. This proves rendering non-use of hidden truth; it does not claim every synthetic variant has an authored gameplay outcome.
...
- The Ticket says storage cable while the profile has at least two cable technologies; private authoring may bind an exact branch, but public projection must preserve both until Evidence distinguishes them.
- Visual inspection establishes access to a connection boundary, not an automatic outcome or permission to repair.
...
- Stop if the authored outcome names a connector or bay that the cited configuration cannot support.
- Stop if the public diagram highlights a loose connector or suppresses a competing device/backplane path.
