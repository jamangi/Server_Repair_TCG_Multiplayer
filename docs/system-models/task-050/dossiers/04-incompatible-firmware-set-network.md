# Incompatible firmware set with network symptoms
...
| Field | Value |
| --- | --- |
| Dossier | `model.incompatible-firmware-set.v1` |
| Released Ticket | `ticket.generated.b34238282822e93980b5f1ad` |
| Fingerprint | `fingerprint.firmware.incompatible_version_set` |
| Ticket snapshot SHA-256 | `face80b0d5c6f6c7f1ef3bb0495c0c6e4105360fa07887fb50cc2dea440cbc50` |
...
- Candidates: `fault.firmware.version_set.incompatible`, `fault.network.cable.disconnected`, `fault.network.cable.failed`, `fault.network.nic.failed`
- Focus: BMC firmware inventory/rollback, UEFI/adapter boundaries, host NIC, physical link, driver handoff, and runtime counters
- Public-safe statement: The visible model keeps firmware, adapter, port, cable, peer, and runtime-counter boundaries separate, leaving both logical and physical network Candidates plausible.
...
- Hidden authored Faults: `fault.firmware.version_set.incompatible`
- Isolation route kinds: `TASK_043_ORACLE_WITNESS`
- Repair procedures: `repair.firmware.restore_compatible_versions`
- Verification procedures: `verify.firmware.compatible_persistent`
- Consistency result: The private view can compare the documented management firmware inventory, correlate a bounded runtime link-counter observation, restore a source-supported prior compatible set, and then verify persistence across the firmware/driver/link boundary. The public view includes the same nodes for every Candidate and never labels a version compatible or incompatible.
...
| DATA | System board → Host NIC | PCIe / host | `claim.r740xd.network-daughter-card` |
| DATA | Host NIC → Port + cable + peer | physical link | `claim.r740xd.network-daughter-card` |
| DATA | OS loader + runtime → Host NIC | driver / counters | `claim.uefi.os-loader-handoff`, `claim.r740xd.network-daughter-card` |
...
**Generated public description:** Standby power reaches the management domain. BMC and lifecycle services initialize independently of the host OS. A host-power request enables the switched board path. Host firmware initializes processor, memory, adapters, and device inventory. UEFI applies boot policy to an available boot option. Firmware hands control to the selected OS loader. The OS exercises storage and network paths while management continues telemetry and event collection. The visible model keeps firmware, adapter, port, cable, peer, and runtime-counter boundaries separate, leaving both logical and physical network Candidates plausible.
...
[Open the standalone SVG](../diagrams/04-incompatible-firmware-set-network.svg). The SVG has a title and long description, labeled relation types, visible keyboard focus for semantic component groups, patterns/markers that survive monochrome or forced-colors rendering, and a deterministic view box.

**Text equivalent:** Ticket ticket.generated.b34238282822e93980b5f1ad has public Symptoms symptom.network.link_flapping, symptom.network.no_link. Its public Candidates are fault.firmware.version_set.incompatible, fault.network.cable.disconnected, fault.network.cable.failed, fault.network.nic.failed. The schematic focus contains System board (host firmware post); BMC + lifecycle (management); UEFI / boot policy (host firmware post); Host NIC (network); Port + cable + peer (network); OS loader + runtime (os handoff). Connections, in reading order: control relation from System board to BMC + lifecycle: host management; containment relation from System board to UEFI / boot policy: firmware surface; lifecycle relation from UEFI / boot policy to OS loader + runtime: boot handoff; data relation from System board to Host NIC: PCIe / host; data relation from Host NIC to Port + cable + peer: physical link; data relation from OS loader + runtime to Host NIC: driver / counters. Line labels and dash patterns distinguish relation types; no node or edge states which Candidate is true.
...
| Repair | `repair.firmware.restore_compatible_versions` | BMC + lifecycle → UEFI / boot policy → Host NIC | changes firmware-bearing roles through a documented rollback/update surface |
| Verification | `verify.firmware.compatible_persistent` | BMC + lifecycle → UEFI / boot policy → Host NIC → OS loader + runtime → Port + cable + peer | checks inventory persistence and bounded runtime link behavior after firmware intervention |

## 10. Hidden Ticket-consistency proof

| Step | Authored action | Definition / target | Profile realization | Authored result reference |
| --- | --- | --- | --- | --- |
| 1 | RUN_DIAGNOSTIC | test.firmware.version_compatibility | BMC + lifecycle → UEFI / boot policy → Host NIC | evidence.fingerprint.firmware.incompatible.version.set.machine.firmware.incompatible.version.set.active.test.firmware.version.compatibility |
| 2 | RUN_DIAGNOSTIC | test.network.link_counter_soak | OS loader + runtime → Host NIC → Port + cable + peer | evidence.fingerprint.firmware.incompatible.version.set.machine.firmware.incompatible.version.set.active.test.network.link.counter.soak |
| 3 | COMMIT_ISOLATION | fault_instance.firmware.incompatible_version_set.root | BMC + lifecycle → UEFI / boot policy → Host NIC | fault_instance.firmware.incompatible_version_set.root |
| 4 | PERFORM_REPAIR | repair.firmware.restore_compatible_versions | BMC + lifecycle → UEFI / boot policy → Host NIC | repair_outcome.fingerprint.firmware.incompatible.version.set |
| 5 | PERFORM_VERIFY | verify.firmware.compatible_persistent | BMC + lifecycle → UEFI / boot policy → Host NIC → OS loader + runtime → Port + cable + peer | verify_outcome.fingerprint.firmware.incompatible.version.set.01.pass |
...
| --- | --- | --- |
| `fault.firmware.version_set.incompatible` | BMC + lifecycle → UEFI / boot policy → Host NIC → OS loader + runtime | The management inventory and documented rollback surface span BIOS, iDRAC, RAID, and NIC firmware while the host driver is a separate handoff boundary. |
| `fault.network.cable.disconnected` | Port + cable + peer | The external port/cable/peer path is visibly distinct. |
| `fault.network.cable.failed` | Port + cable + peer | The same physical path supports a failed-cable hypothesis without labeling link state. |
| `fault.network.nic.failed` | Host NIC | The host NIC is a documented serviceable adapter role. |

Differential variants tested with this exact public input: `fault.firmware.version_set.incompatible`, `fault.network.cable.disconnected`, `fault.network.cable.failed`, `fault.network.nic.failed`. The focused validator substitutes each Candidate as synthetic hidden truth and proves the public narrative, text equivalent, and SVG remain byte-identical. This proves rendering non-use of hidden truth; it does not claim every synthetic variant has an authored gameplay outcome.
...
- No specific compatible version tuple is invented; the manufacturer rollback source proves the surface and constraints, not the Ticket's authored compatibility matrix.
- Link counter behavior and driver state are bounded OS-runtime observations; they do not make the diagram a network protocol model.
...
- Stop if a Ticket needs a concrete version tuple without a product support matrix or authored source ledger entry.
- Stop if rollback is represented as universally available; stored previous-version and component-support constraints must remain explicit.
- Stop if the public view displays version mismatch, bad counters, or link failure before Evidence authorizes it.
