...
| Field | Value |
| --- | --- |
| Dossier | `model.corrupt-bmc-firmware.v1` |
| Released Ticket | `ticket.generated.f32b85cbf2054fdf0114f42a` |
| Fingerprint | `fingerprint.management.corrupt_bmc_firmware` |
| Ticket snapshot SHA-256 | `761016e56ceb47a585727555f64bac47b103933cddbafc27a5c385f402851b01` |
...
- Candidates: `fault.firmware.version_set.incompatible`, `fault.management.bmc_firmware.corrupt`
- Focus: Standby management initialization, BMC recovery state, external recovery image, bootloader reprogramming, and host-management reconnection
- Public-safe statement: The visible model separates management recovery from ordinary version compatibility while retaining the host board, UEFI, and runtime interfaces that both public Candidates can affect.
...
- Hidden authored Faults: `fault.management.bmc_firmware.corrupt`
- Isolation route kinds: `TASK_043_ORACLE_WITNESS`
- Repair procedures: `repair.management.recover_bmc_firmware`
- Verification procedures: `verify.management.bmc_functional`
- Consistency result: The private view can inspect the documented recovery-state boundary, supply an external recovery image through the vendor procedure, allow the bootloader to reprogram and reboot the management controller, and then verify both management responsiveness and bounded host coordination. The public projection does not show recovery mode, corruption, image presence, or result.
...
| --- | --- | --- | --- |
| POWER | PSU pair → System board | standby + switched | `claim.r740xd.dual-psu` |
...
**Generated public description:** Standby power reaches the management domain. BMC and lifecycle services initialize independently of the host OS. A host-power request enables the switched board path. Host firmware initializes processor, memory, adapters, and device inventory. UEFI applies boot policy to an available boot option. Firmware hands control to the selected OS loader. The OS exercises storage and network paths while management continues telemetry and event collection. The visible model separates management recovery from ordinary version compatibility while retaining the host board, UEFI, and runtime interfaces that both public Candidates can affect.
...
[Open the standalone SVG](../diagrams/05-corrupt-bmc-firmware.svg). The SVG has a title and long description, labeled relation types, visible keyboard focus for semantic component groups, patterns/markers that survive monochrome or forced-colors rendering, and a deterministic view box.

**Text equivalent:** Ticket ticket.generated.f32b85cbf2054fdf0114f42a has public Symptoms symptom.management.bmc_not_responding, symptom.firmware.update_failed. Its public Candidates are fault.firmware.version_set.incompatible, fault.management.bmc_firmware.corrupt. The schematic focus contains PSU pair (power); System board (host firmware post); BMC + lifecycle (management); UEFI / boot policy (host firmware post); OS loader + runtime (os handoff). Connections, in reading order: power relation from PSU pair to System board: standby + switched; control relation from System board to BMC + lifecycle: host management; containment relation from System board to UEFI / boot policy: firmware surface; lifecycle relation from UEFI / boot policy to OS loader + runtime: boot handoff. Line labels and dash patterns distinguish relation types; no node or edge states which Candidate is true.
...
| Test | `test.management.bmc_recovery_state` | BMC + lifecycle → System board | queries the documented management recovery boundary |
| Repair | `repair.management.recover_bmc_firmware` | BMC + lifecycle → System board | applies the documented external-image recovery path to the management firmware role |
| Verification | `verify.management.bmc_functional` | BMC + lifecycle → System board → UEFI / boot policy → OS loader + runtime | checks management responsiveness and bounded host coordination after recovery |

## 10. Hidden Ticket-consistency proof

| Step | Authored action | Definition / target | Profile realization | Authored result reference |
| --- | --- | --- | --- | --- |
| 1 | RUN_DIAGNOSTIC | test.management.bmc_recovery_state | BMC + lifecycle → System board | evidence.fingerprint.management.corrupt.bmc.firmware.machine.management.corrupt.bmc.firmware.active.test.management.bmc.recovery.state |
| 2 | COMMIT_ISOLATION | fault_instance.management.corrupt_bmc_firmware.root | BMC + lifecycle → System board | fault_instance.management.corrupt_bmc_firmware.root |
| 3 | PERFORM_REPAIR | repair.management.recover_bmc_firmware | BMC + lifecycle → System board | repair_outcome.fingerprint.management.corrupt.bmc.firmware |
| 4 | PERFORM_VERIFY | verify.management.bmc_functional | BMC + lifecycle → System board → UEFI / boot policy → OS loader + runtime | verify_outcome.fingerprint.management.corrupt.bmc.firmware.01.pass |
...
| --- | --- | --- |
| `fault.firmware.version_set.incompatible` | BMC + lifecycle → UEFI / boot policy → OS loader + runtime | The documented firmware inventory/rollback surface crosses management, BIOS, and host-facing component versions. |
| `fault.management.bmc_firmware.corrupt` | BMC + lifecycle → System board | The documented iDRAC9 recovery path establishes a recoverable management-firmware role on the board service boundary. |

Differential variants tested with this exact public input: `fault.firmware.version_set.incompatible`, `fault.management.bmc_firmware.corrupt`. The focused validator substitutes each Candidate as synthetic hidden truth and proves the public narrative, text equivalent, and SVG remain byte-identical. This proves rendering non-use of hidden truth; it does not claim every synthetic variant has an authored gameplay outcome.
...
- The recovery image and bootloader are logical nodes described in prose/text equivalent rather than extra diagram boxes; the cited procedure supplies their exact recovery roles.
- The BMC is not labeled as a separate replaceable board; the pilot distinguishes firmware recovery from board replacement.
...
- Stop if a proposed recovery step uses a filename, media format, or controller generation other than the applicable manufacturer procedure.
- Stop if recovery success is inferred from image presence rather than the Ticket's authored Verification.
- Stop if the public view reveals recovery state or distinguishes the two Candidates using private authoring data.
