...
TASK-031 is the next active contract: catalogue every SFX-capable Library, Play, Profile, Settings, active-Match, and Story interaction into maintainable human- and machine-readable semantic maps. TASK-032 then adds one accessible app-wide audio service and a persisted 0–100 SFX volume setting. The locally supplied JDSherbert pack is not committed: its license requires credit and forbids raw-file redistribution, so public repository/static-web integration waits for explicit written permission covering that delivery model or a rights-compatible replacement pack. See the [task index](docs/tasks/INDEX.md) and [source/license review](docs/audio/SFX_SOURCE_REVIEW.md).
...
- [`docs/audio/`](docs/audio/) — UI sound source/license review and the future semantic interaction/asset catalogues used by TASK-031 and TASK-032.
