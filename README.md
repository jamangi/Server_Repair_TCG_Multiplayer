...
TASK-031 is the next active contract: catalogue every SFX-capable Library, Play, Profile, Settings, active-Match, and Story interaction plus the repository's procedural recipe candidates into maintainable human- and machine-readable maps. TASK-032 then adds one accessible browser-native Web Audio service and a persisted 0–100 SFX volume setting. The strategy synthesizes short cues from oscillators, generated noise, envelopes, filters, and bounded delay without sampled audio, a framework, an external audio dependency, or a runtime download. See the [task index](docs/tasks/INDEX.md) and [procedural synthesis review](docs/audio/SFX_SYNTHESIS_REFERENCE.md).
...
- [`docs/audio/`](docs/audio/) — procedural UI sound synthesis review and the future semantic interaction/recipe catalogues used by TASK-031 and TASK-032.
