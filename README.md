...
- a deterministic declarative Story runtime with strict typed statements and conditions, bounded jump/call/return control flow, layered display commands, stable checkpoint digests, normalized Match boundaries, explicit v2-to-v4 local/export migration, and a non-canon proof fixture;
- the characterization-v2 canonical campaign-one story, *The Quiet Cascade*: four chapters, six solvability-proved real Matches covering all twelve current causal fingerprints, 335 reachable statements, 48 route/outcome traversals, three ending bands, original/context/final dialogue proof, and safe v1 checkpoint migration;
...
TASK-037 is active. Static inspection confirms that Shift 6, *The Quiet Cascade*, already exists with three generated Tickets, exact deck requirements, pre/post-Match checkpoints, return dialogue, and ending routes. TASK-037 therefore treats the reported inert Continue control after five completed Shifts as a saved-progress/launch-boundary defect and adds an explicit campaign-complete “more content in development” state after the real ending.

The planned follow-on sequence is TASK-038 through TASK-046: completed-episode replay; an exact six-Shift domain coverage audit; a reusable expansion protocol; calculation and research of only the additional real-world cases actually needed; coherent domain/network integration; a six-episode Story+Match blueprint; scripts; gap-only original art; and one versioned release. The [task index](docs/tasks/INDEX.md) contains the dependency order. [`STORY-009 A`](docs/design/decisions/APPROVALS.md#story-009--completed-episode-replay-semantics--a-approved-2026-08-28) approves replay as an isolated, non-scoring review session that cannot rewrite canonical Story progress or Profile statistics.
