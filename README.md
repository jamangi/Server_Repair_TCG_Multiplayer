...
TASK-056 is the current authorized maintenance boundary. It owns five reproducible legacy Playwright failures—two obsolete scenario drivers, two stale `12 of 12` coverage assertions, and one fixed Ticket-title accessibility allowlist—and a separate investigate-first TASK-015 focus failure seen only in a concurrent full-suite run. The task requires zero unexpected failures in both serial and four-worker repository sweeps without retries, skips, arbitrary waits, global serialization, or weakened keyboard/accessibility assertions. See the [task contract](docs/tasks/TASK-056-restore-browser-baseline-and-investigate-focus-flake.md) and [source QA record](docs/system-models/task-055/BROWSER_QA.md).
