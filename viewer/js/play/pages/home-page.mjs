...
import { deckComposition } from '../catalog-service.mjs';
...
export function renderHome(root, context) {
  ...
  const duplicateDisclosure = settings.starting_ticket_count > 1;
      ...
      <div class="home-grid">
        ...
        <aside class="shift-panel" data-route-reveal>
          ...
          <section class="active-deck-summary" aria-labelledby="active-deck-heading">
            ...
            ${activeDeck ? `<p>${activeDeck.card_definition_ids.length} Cards · legal for solo play</p><div class="composition-row">${compositionMarkup(composition)}</div>` : '<p class="status-warning">Choose or build a legal 30-card deck before starting.</p>'}
          ...
          <p id="duplicate-disclosure" class="duplicate-disclosure"${duplicateDisclosure ? '' : ' hidden'}><strong>Repeat-scenario disclosure:</strong> this training profile permits repeated templates in queues above one Ticket; queues above three necessarily repeat because the pinned pack contains three causal fingerprints. Every Ticket remains a separate machine instance and closure.</p>
          <button id="start-solo" class="play-button play-button--primary play-button--large" type="button"${activeDeck && !context.snapshot.recovery_required ? '' : ' disabled'}>
